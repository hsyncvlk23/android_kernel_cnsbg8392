...
#define CALI_FILE "/persist-lg/sensor/sensor_cal_data.txt"
#endif
...
static struct data_resolution bma255_data_resolution[1] = {
 /* combination by {FULL_RES,RANGE}*/
    ...
    {{ 0, 9}, 1024},   // dataformat +/-2g  in 10-bit resolution;  { 3, 9} = 3.9= (2*2*1000)/(2^10);  256 = (2^10)/(2*2)          
};
/*----------------------------------------------------------------------------*/
static struct data_resolution bma255_offset_resolution = {{0, 9}, 1024};
...
static ssize_t show_mibtest_value(struct device_driver *ddri, char *buf)
{
	GSE_ERR("show_mibtest_value: mib custom function\n");
	return snprintf(buf, PAGE_SIZE, "BMA255 Chip\n");        
}
static DRIVER_ATTR(mmtest1,   S_IWUSR | S_IRUGO, show_mibtest_value,      NULL);
static DRIVER_ATTR(mmtest11,   S_IWUSR | S_IRUGO, show_mibtest_value,      NULL);
static DRIVER_ATTR(mmtest2,   S_IWUSR | S_IRUGO, show_mibtest_value,      NULL);
...
static int bma255_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    //driver_create_file(&bma255_gsensor_driver.driver, &driver_attr_mmtest1);
    ...
    //driver_create_file(&bma255_gsensor_driver.driver, &driver_attr_mmtest2);
...
exit_init_failed:
    ...
    //driver_create_file(&bma255_gsensor_driver.driver, &driver_attr_mmtest11);
...
static int __init bma255_init(void)
{
	...
	GSE_FUN();
	bma255_i2c_addr = BMA255_I2C_ADDR;
