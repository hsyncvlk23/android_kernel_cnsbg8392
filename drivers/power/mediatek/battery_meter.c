...
extern int gesture_wakeup_function;
...
R_PROFILE_STRUC_P fgauge_get_profile_r_table(kal_uint32 temperature)//johnny[2015.06.10]modify function
{
		/*switch (temperature) {
	case batt_meter_cust_data.temperature_t0:
...
kal_int32 fgauge_read_r_bat_by_v(kal_int32 voltage)
{
	...
	if (profile_p == NULL) {
		...
		return (profile_p + 0)->resistance-30;//johnny[2015.6.10]rbatt modify
	}
	...
	if (voltage < (profile_p + saddles - 1)->voltage) {
		return (profile_p + saddles - 1)->resistance-30;//johnny[2015.6.10]rbatt modify
	}
	...
	return ret_r-30;//johnny[2015.6.10]rbatt modify
}
...
void oam_run(void)
{
	...
	//get_monotonic_boottime(&now_time);
        getrawmonotonic(&now_time);//johnny[2015.06.29]using kernel time for oam_run
	...
	bm_print(BM_LOG_CRTI, "[oam_run_time] delta time=%d\n", delta_time);
        pr_info("johnny tp wakeup:%d %x\n",gesture_wakeup_function,gesture_wakeup_function);
#if defined(SW_OAM_INIT_V2)
	printk(bootbuf);
#endif
	...
	fgauge_construct_table_by_temp();
        gFG_BATT_CAPACITY_aging = fgauge_get_Q_max(force_get_tbat(KAL_FALSE));//johnny add
#if defined(CUST_CAPACITY_OCV2CV_TRANSFORM)
...
/*johnny[2015.6.10], add for user space debug*/
static ssize_t show_oam_i_1(struct device *dev,struct device_attribute *attr, char *buf)
{
    bm_print(BM_LOG_CRTI, "johnny debug, oam_i_1 : %d\n", oam_i_1);
    return sprintf(buf, "%d\n", oam_i_1);
}
static ssize_t store_oam_i_1(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    return size;
}
static DEVICE_ATTR(oam_i_1, 0664, show_oam_i_1, store_oam_i_1);
static ssize_t show_oam_i_2(struct device *dev,struct device_attribute *attr, char *buf)
{
    bm_print(BM_LOG_CRTI, "johnny debug, oam_i_2 : %d\n", oam_i_2);
    return sprintf(buf, "%d\n", oam_i_2);
}
static ssize_t store_oam_i_2(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    return size;
}
static DEVICE_ATTR(oam_i_2, 0664, show_oam_i_2, store_oam_i_2);
static ssize_t show_oam_r_1(struct device *dev,struct device_attribute *attr, char *buf)
{
    bm_print(BM_LOG_CRTI, "johnny debug, oam_r_1 : %d\n", oam_r_1);
    return sprintf(buf, "%d\n", oam_r_1);
}
static ssize_t store_oam_r_1(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    return size;
}
static DEVICE_ATTR(oam_r_1, 0664, show_oam_r_1, store_oam_r_1);
static ssize_t show_oam_r_2(struct device *dev,struct device_attribute *attr, char *buf)
{
    bm_print(BM_LOG_CRTI, "johnny debug, oam_r_2 : %d\n", oam_r_2);
    return sprintf(buf, "%d\n", oam_r_2);
}
static ssize_t store_oam_r_2(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    return size;
}
static DEVICE_ATTR(oam_r_2, 0664, show_oam_r_2, store_oam_r_2);
static ssize_t show_oam_car_1(struct device *dev,struct device_attribute *attr, char *buf)
{
    bm_print(BM_LOG_CRTI, "johnny debug, oam_car_1 : %d\n", oam_car_1);
    return sprintf(buf, "%d\n", oam_car_1);
}
static ssize_t store_oam_car_1(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    return size;
}
static DEVICE_ATTR(oam_car_1, 0664, show_oam_car_1, store_oam_car_1);
static ssize_t show_oam_car_2(struct device *dev,struct device_attribute *attr, char *buf)
{
    bm_print(BM_LOG_CRTI, "johnny debug, oam_car_2 : %d\n", oam_car_2);
    return sprintf(buf, "%d\n", oam_car_2);
}
static ssize_t store_oam_car_2(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    return size;
}
static DEVICE_ATTR(oam_car_2, 0664, show_oam_car_2, store_oam_car_2);

static ssize_t show_oam_ocv_1(struct device *dev,struct device_attribute *attr, char *buf)
{
    bm_print(BM_LOG_CRTI, "johnny debug, oam_ocv_1 : %d\n", oam_v_ocv_1);
    return sprintf(buf, "%d\n", oam_v_ocv_1);
}
static ssize_t store_oam_ocv_1(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    return size;
}
static DEVICE_ATTR(oam_ocv_1, 0664, show_oam_ocv_1, store_oam_ocv_1);
static ssize_t show_oam_ocv_2(struct device *dev,struct device_attribute *attr, char *buf)
{
    bm_print(BM_LOG_CRTI, "johnny debug, oam_v_ocv_2 : %d\n", oam_v_ocv_2);
    return sprintf(buf, "%d\n", oam_v_ocv_2);
}
static ssize_t store_oam_ocv_2(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    return size;
}
static DEVICE_ATTR(oam_ocv_2, 0664, show_oam_ocv_2, store_oam_ocv_2);
#ifdef MTK_MULTI_BAT_PROFILE_SUPPORT
#ifdef MTK_GET_BATTERY_ID_BY_AUXADC
static ssize_t show_batt_id(struct device *dev,struct device_attribute *attr, char *buf)
{

    bm_print(BM_LOG_CRTI, "johnny debug, batt_id : %d\n", fgauge_get_profile_id());
    return sprintf(buf, "%d\n", fgauge_get_profile_id());
}
static ssize_t store_batt_id(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    return size;
}
static DEVICE_ATTR(batt_id, 0664, show_batt_id, store_batt_id);
#endif // MTK_GET_BATTERY_ID_BY_AUXADC
#endif
/*johnny[2015.6.10], add for user space debug  END*/
/* ============================================================ // */
static int battery_meter_probe(struct platform_device *dev)
{
	...
	ret_device_file =
	    device_create_file(&(dev->dev), &dev_attr_FG_g_fg_dbg_percentage_voltmode);
        	/*johnny[2015.6.10], add sys node start*/
            ret_device_file = device_create_file(&(dev->dev), &dev_attr_oam_i_1);
            ret_device_file = device_create_file(&(dev->dev), &dev_attr_oam_i_2);
            ret_device_file = device_create_file(&(dev->dev), &dev_attr_oam_r_1);
            ret_device_file = device_create_file(&(dev->dev), &dev_attr_oam_r_2);
            ret_device_file = device_create_file(&(dev->dev), &dev_attr_oam_car_1);
            ret_device_file = device_create_file(&(dev->dev), &dev_attr_oam_car_2);
            ret_device_file = device_create_file(&(dev->dev), &dev_attr_oam_ocv_1);
            ret_device_file = device_create_file(&(dev->dev), &dev_attr_oam_ocv_2);
#ifdef MTK_MULTI_BAT_PROFILE_SUPPORT
#ifdef MTK_GET_BATTERY_ID_BY_AUXADC
            ret_device_file = device_create_file(&(dev->dev), &dev_attr_batt_id);
#endif
#endif
    /*johnny[2015.5.20], add sys node END*/
...
static int battery_meter_resume(struct platform_device *dev)
{
	...
	if (_g_bat_sleep_total_time > 3600) {	// 1hr
		if (hw_ocv_after_sleep < g_hw_ocv_before_sleep) {
			...
			oam_car_1 = 0;
			oam_car_2 = 0;
			bm_print(BM_LOG_CRTI,"johnny,read new OCV:%d\n",hw_ocv_after_sleep);
		} else {
		        if(gesture_wakeup_function == 0){
			//johnny,[2015.06.09]modify the suspend current
			    oam_car_1 = oam_car_1 + (120* (rtc_time_after_sleep.tv_sec - g_rtc_time_before_sleep.tv_sec)/3600); //0.1mAh
                            oam_car_2 = oam_car_2 + (120* (rtc_time_after_sleep.tv_sec - g_rtc_time_before_sleep.tv_sec)/3600); //0.1mAh
                            bm_print(BM_LOG_CRTI,"johnny,EZ close, suspend discharge %ld mAh\n",(120* (rtc_time_after_sleep.tv_sec - g_rtc_time_before_sleep.tv_sec)/3600));
		        }
		        else{
                            oam_car_1 = oam_car_1 + (300* (rtc_time_after_sleep.tv_sec - g_rtc_time_before_sleep.tv_sec)/3600); //0.1mAh
                            oam_car_2 = oam_car_2 + (300* (rtc_time_after_sleep.tv_sec - g_rtc_time_before_sleep.tv_sec)/3600); //0.1mAh
                            bm_print(BM_LOG_CRTI,"johnny,EZ open, suspend discharge %ld mAh\n",(300* (rtc_time_after_sleep.tv_sec - g_rtc_time_before_sleep.tv_sec)/3600));
		        }
		}
	}
    //FIXME*/
	...
	if (rtc_time_after_sleep.tv_sec - g_rtc_time_before_sleep.tv_sec > 1800 &&
		...
		/*if ((DOD_hwocv < 30 || DOD_hwocv > 70) &&
			...
			bm_print(BM_LOG_CRTI,
				"[johnny] reset to HWOCV. dod_ocv(%d) dod_now(%d)\n",
				DOD_hwocv, oam_d_2);
		/*} else {
			if(gesture_wakeup_function == 0){
			//johnny,[2015.06.09]modify the suspend current
