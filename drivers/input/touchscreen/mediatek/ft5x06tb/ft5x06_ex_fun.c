...
static ssize_t ft5x0x_debug_write (struct file *filp, const char __user *userbuf, size_t count, loff_t *ppos)
{
...
static const struct file_operations ft5x0x_proc_t_fops = {.write = ft5x0x_debug_write, .read = ft5x0x_debug_read, .open = simple_open, .owner = THIS_MODULE,};
int ft5x0x_create_apk_debug_channel(struct i2c_client * client)
{
	ft5x0x_proc_entry = proc_create_data(PROC_NAME, 0666, ft5x0x_proc_entry, &ft5x0x_proc_t_fops, (void *)client);
