...
int fts_5x22_ctpm_fw_upgrade(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth,u32 *debug_info);
...
#define UPGRADE_RETRY_LOOP	5
unsigned int upgrade_debug_info[UPGRADE_RETRY_LOOP];

struct i2c_client *G_Client;
static unsigned char CTPM_FW[] = {
	#include "5726_0x01_0x12_20150705_app.i"
};
//kernel 3.10
//struct i2c_client *G_Client;
...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
	...
	*/
        ...
        TPD_DMESG ("Tommy fts_updateinfo_curr.CHIP_ID = %x\r\n",fts_updateinfo_curr.CHIP_ID);
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x58))
	{
		...
		pbt_buf = CTPM_FW;
		/*call the upgrade function*/
		memset(upgrade_debug_info, 0, sizeof(upgrade_debug_info));

		for(i = 0; i < UPGRADE_RETRY_LOOP; i++){
			i_ret = fts_5x22_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW), &upgrade_debug_info[i]);
			if (i_ret != 0) 
			{
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	*/
	if((uc_tp_fm_ver < uc_host_fm_ver) || (uc_tp_fm_ver >= 0xe0) || (uc_tp_fm_ver == 0x00) || (uc_tp_fm_ver == 0xa6) ) 
	{
		msleep(100);
...
int fts_5x26_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	reg_val[0] = reg_val[1] = 0x00;
	fts_i2c_Read(client, auc_i2c_write_buf, 1, reg_val, 1);
	TPD_DMESG(KERN_WARNING "Checksum FT5X26:%X %X \n", reg_val[0], bt_ecc);
	if (reg_val[0] != bt_ecc) 
	{
		dev_err(&client->dev, "[FTS]--ecc error! FW=%02x bt_ecc=%02x\n",reg_val[0],bt_ecc);
		return -EIO;
...
int fts_5x22_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth, u32 *debug_info)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		auc_i2c_write_buf[1] = auc_i2c_write_buf[2] = auc_i2c_write_buf[3] = 0x00;
		reg_val[0] = reg_val[1] = 0x00;
		i_ret = fts_i2c_Read(client, auc_i2c_write_buf, 4, reg_val, 2);
		if (i_ret < 0) 
			*debug_info |= BIT(5);
	...
	FTS_DBG("Step 4:erase app and panel paramenter area\n");
	auc_i2c_write_buf[0] = 0x61;
	i_ret = fts_i2c_Write(client, auc_i2c_write_buf, 1);		/*erase app area*/	/*: trigger erase command */
	if (i_ret < 0) 
		*debug_info |= BIT(8);
	...
	TPD_DMESG("[FTS][%s] erase app area reg_val[0] = %x reg_val[1] = %x \n", __func__, reg_val[0], reg_val[1]);
	...
	for (j = 0; j < packet_number; j++) 
	{
		...
		TPD_DMESG("[FTS][%s] bt_ecc = %x \n", __func__, bt_ecc);
		if (bt_ecc != bt_ecc_check)
			TPD_DMESG("[FTS][%s] Host checksum error bt_ecc_check = %x \n", __func__, bt_ecc_check);
		...
		for (i = 0; i < 30; i++) 
		{
			...
			TPD_DMESG("[FTS][%s] reg_val[0] = %x reg_val[1] = %x \n", __func__, reg_val[0], reg_val[1]);
			msleep(1);
		}
	}
	if ((dw_lenth) % FTS_PACKET_LENGTH > 0) 
	...
	TPD_DMESG(KERN_WARNING "checksum %X %X \n", reg_val[0], bt_ecc);
	/*********Step 7: reset the new FW***********************/
	FTS_DBG("Step 7: reset the new FW\n");
	auc_i2c_write_buf[0] = 0x07;
	fts_i2c_Write(client, auc_i2c_write_buf, 1);
				       ...
				       char *firmware_name)
{
	...
	{
		upgrade_debug_info[0] = 0;
		i_ret =  fts_5x22_ctpm_fw_upgrade(client, pbt_buf, fwsize, &upgrade_debug_info[0]);
	}
	else if ((fts_updateinfo_curr.CHIP_ID==0x59))
	{
...
//Tommy0513 -----------------------------------
static ssize_t nkt_tpfwver_show(struct device *dev,
                                        struct device_attribute *attr,
                                        char *buf)
{
        ssize_t num_read_chars = 0;
        ...
        {
                num_read_chars = snprintf(buf, 100, "%d", fwver);
        }
        mutex_unlock(&g_device_mutex);
...
static ssize_t nkt_tpfwver_store(struct device *dev,
                                        struct device_attribute *attr,
                                        const char *buf, size_t count)
{
        /*place holder for future use*/
...
//Tommy0604 -----------------------------------
static ssize_t nkt_tpfwerr_show(struct device *dev,
                                        struct device_attribute *attr,
                                        char *buf)
{

        return sprintf(buf, "%08X %08X %08X %08X %08X\n", upgrade_debug_info[0], upgrade_debug_info[1], upgrade_debug_info[2], upgrade_debug_info[3], upgrade_debug_info[4] );
}

static ssize_t nkt_tpfwerr_store(struct device *dev,
                                        struct device_attribute *attr,
                                        const char *buf, size_t count)
{
        /*place holder for future use*/
...
//Tommy0513
static DEVICE_ATTR(nkttpfwver, S_IRUGO | S_IWUSR, nkt_tpfwver_show,nkt_tpfwver_store);

//Tommy0604
static DEVICE_ATTR(nkttpfwerr, S_IRUGO | S_IWUSR, nkt_tpfwerr_show,nkt_tpfwerr_store);
...
static struct attribute *fts_attributes[] = {
	...
	&dev_attr_nkttpfwver.attr,
	&dev_attr_nkttpfwerr.attr,
	NULL
};
